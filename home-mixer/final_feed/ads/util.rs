use super::super::feed_item::{Advertisement, FeedItem, FeedItemContent};
...
pub(super) const MIN_REQUESTED_GAP: usize = 3;
pub(super) const DEFAULT_SPACING: AdSpacing = AdSpacing {
    requested: 3,
    min: 2,
};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) struct AdSpacing {
    pub(super) requested: usize,
    pub(super) min: usize,
}

pub(super) fn compute_spacing(advertisements: &[FeedItem]) -> AdSpacing {
...
pub(super) fn find_safe_gaps(posts: &[FeedItem]) -> Vec<usize> {
    (1..posts.len())
...
pub(super) fn requested_ad_position(item: &FeedItem) -> usize {
    match &item.content {
...
pub(super) fn interleave_at_gaps(
    posts: Vec<FeedItem>,
...
pub(super) fn is_safe_for_adjacency(item: &FeedItem) -> bool {
    matches!(
...
pub(super) fn should_drop_bsr_low(
...
pub(super) fn should_drop_handle(
    advertisement: &Advertisement,
    above: Option<&FeedItem>,
    below: Option<&FeedItem>,
) -> bool {
...
pub(super) fn should_drop_keyword(
    advertisement: &Advertisement,
    above: Option<&FeedItem>,
    below: Option<&FeedItem>,
) -> bool {
