...
flowchart TD
    ...
    C --> C0["MRPYQ_RECOMMENDATION_DATA_ADDR<br/>真实业务必填"]
    C --> C1["Phoenix gRPC 地址"]
...
| `HOME_MIXER_MODE` | `runtime_config.rs` | 运行意图：`degraded` / `production_ready` | 默认 `degraded`（需 `MRPYQ_RECOMMENDATION_DATA_ADDR`）；调用方身份、TES、UAS 事件合同、Strato、VF、网内 / 兜底、Phoenix 元数据、served 落库合同未验收前，`production_ready` 拒绝启动 |
...
| `HOME_MIXER_REDIS_URL` | `runtime_config.rs` / `server.rs` | 共享 FeedStateStore 的 Redis 地址 | 真实业务模式必填；连接失败会拒绝启动 |
...
| `UAS_REDIS_URL` | `runtime_config.rs`（`UasConfig`） | UAS 投影 job 与 Home Mixer 共用的 Redis 地址 | 缺省复用 `HOME_MIXER_REDIS_URL`；job 必须能从两者之一取到地址。连接失败拒绝启动 |
...
| `SERVED_EVENTS_KAFKA_BROKERS` / `SERVED_EVENTS_KAFKA_TOPIC` | `clients/served_candidates_sink.rs` | 装配 `ServedCandidatesKafkaSideEffect`，把每次请求的服务端曝光事件发到该 topic（分区键 `viewer_id`，`enable.idempotence=true`） | 两者必须同时设置；未设置时不装配曝光日志并告警。Kafka sink 需要 `--features kafka` 构建（TLS / SCRAM 需 `kafka-ssl`），否则启动失败。关停时先等在途 side effect，再 flush producer |
...
| `HOME_MIXER_ENABLE_UNSIGNED_CACHED_POSTS` | `feature_policy.rs` / `runtime_config.rs` / `server.rs` | 允许请求直接携带未签名 `cached_posts` fixture | 当前不支持；生产缓存必须使用服务端状态或签名/opaque 合同 |
| `HOME_MIXER_ENABLE_VM_RANKER` | `feature_policy.rs` | 显式启用 VM Ranker 二次重排 Scorer | 默认关闭；整数 ID 合同不属于当前生产主线，缺少真实 ObjectId/协议适配时保持关闭 |
| `HOME_MIXER_ENABLE_AUTHOR_COLD_START` | `feature_policy.rs` | 启用低曝光新作者提升，并在 scorer 前补作者粉丝数 | 默认关闭；完成真实 adapter 和候选字段验收前保持关闭 |
...
| `HOME_MIXER_VF_FAILURE_POLICY` | `feature_policy.rs` / `filters/vf_filter.rs` | VF 请求失败/超时、`Unchecked`、成功响应缺帖时的候选保留策略：`fail_closed` 全丢弃，`in_network_only` 仅保留 `in_network == Some(true)`，`allow_all` 全保留 | 默认 `fail_closed`；大小写不敏感、trim 后解析，未知值告警并按 `fail_closed` |
...
Phoenix 两个地址分别指向 xrex 的 ranking / retrieval 服务。当前仓库未提供旧演示网关或对应部署清单；xrex 服务使用 `phoenix/xrex/inference/launch_inference.py` 启动，且尚未与 Home Mixer 当前请求契约完成适配。正式接入前必须完成协议适配和 contract test。

### 3.2 可选集成启用规则

`HomeMixerConfig::from_env()` 在进程装配时一次性生成 `HomeMixerFeatures`。Source、SideEffect 和其他业务组件不直接读取环境变量。

`HomeMixerConfig.feed_state` 明确选择 Redis，由 `HomeMixerServer::build` 创建实例。真实业务模式缺少 Redis 配置时直接拒绝启动。Redis adapter 使用异步多路复用连接，同一连接可处理多个并发请求，已移除 `HOME_MIXER_REDIS_POOL_SIZE`。当前客户端支持单节点或提供兼容端点的代理，不支持原生 Cluster 路由。
...
`HomeMixerConfig.uas`（`UasConfig`）同样在启动时一次解析：取 `UAS_REDIS_URL`，缺省复用 `HOME_MIXER_REDIS_URL`，因此通过 `HomeMixerConfig::from_env()` 启动的业务服务总是装配 Redis UAS adapter。只有直接调用 `PhoenixCandidatePipeline::assemble_for_mode` 且两个地址都没有配置时才得到 `Disabled`（空序列、Phoenix 整体跳过），装配层会打 warn。UAS Redis 连接失败同样拒绝启动。
...
请求缓存 SideEffect 即使显式开启，`MrpyqStratoClient` 也会因为 mrpyq 没有 served 状态写契约而拒绝持久化写入。必须完成人工接入和持久化验收后，才能把它视为生产数据闭环。
...
- `degraded` 注入 `MrpyqFirstStageEligibilityClient`，它只承载 mrpyq 一级 `recommendation_eligible`，没有 viewer 级判定
...
对应 `AuthorColdStartScorer`，需同时设置 `HOME_MIXER_ENABLE_AUTHOR_COLD_START`，并完成真实 adapter 验收：
