...
- 上游调用：`home_mixer_client_calls_total{client,method,result}`、`home_mixer_client_call_duration_seconds{client,method}`，按依赖拆分：mrpyq（`ListRecommendationCandidates` / `BatchGetRecommendationContents` / `GetViewerRelations`）、Phoenix（`PredictNextActions` / `Retrieve`）、ID Registry（`resolve_grpc` / `allocate_grpc` / `reverse_grpc`）和 Redis（feed-state `load` / `record`、UAS `read` / `write`）。`result` 至少区分 `ok` / `error`，Registry 额外使用 `mapping_miss` / `reverse_miss`；慢依赖归因不用再对阶段耗时与日志。uas-worker 的 registry 同时注册 `id_registry` allocation 与 `redis_uas` write 系列，可直接区分身份注册失败和投影 Redis 失败。
- 单请求身份账单：主请求路径结束时会在 debug 日志输出 `main_path_identity_registry_snapshot allocate_batches/allocate_ids/resolve_batches/resolve_ids/reverse_batches/reverse_ids`。这些计数来自主路径的 request-local `IdentityContext`，只统计主路径实际触发的 Registry backend batch/ID 数；异步 served-candidates side effect 完成时另行输出 `side_effect_identity_registry_snapshot ...`。side effect 共享 request-local cache 和 single-flight 状态，但使用独立的统计计数器与 `SIDE_EFFECT_TIMEOUT_MS` 绝对预算，因此两个快照是相互独立的账单，主请求耗尽预算不会让曝光事件的身份反查必然立即失败。两类快照都不把 request_id 作为 Prometheus label；跨请求总量仍以 `home_mixer_client_calls_total` 为准。
...
2. 网内源拿不到内容：检查 `mrpyq adapter recall ...` 日志里的 `ready=false`、`posts=0` 或召回预算耗尽
