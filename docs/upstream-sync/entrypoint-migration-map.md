...
| Pre-selection Hydrators | 10 | 8 | InNetwork, CoreData, Quote, VideoDuration, HasMedia, Subscription, FilteredTopics, and LanguageCode use upstream entry boundaries; a shared TES provider prevents duplicate core/media batches. Gizmoduck runs post-selection instead (`U2`, see below). BlockedBy remains `U3`. |
