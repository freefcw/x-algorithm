...
4. URT is unmigrated and remains contract-blocked. `47c1bcd:` upstream published `home-mixer/util/urt/` (17 files) covering cursors, controller data, feedback, and post/ad/frame serialization, so the *marshalling logic* is now readable. The *wire schema* is not: every one of those files imports `xai_urt_thrift`, which is absent from the upstream tree, so migrating URT would mean inventing the URT thrift schema locally — exactly what the `U3` rule forbids. This is therefore not a sizing question. Authoritative disposition is `45b48ba-capability-inventory.md` §4 `U1`: no local response/data entry exists, so migrating one segment would only create uncalled code.
