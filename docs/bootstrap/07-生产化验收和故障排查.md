...
> **状态**：`runbook`
>
> 当前仓库能跑通 Demo，但 `production_ready` 模式会主动拒绝启动。本文给出从真实 MVP 到生产系统还需要补齐什么，以及出现问题时如何定位。
...
### 7.6 Business Feed 返回 `failed_precondition`

检查：

```bash
echo "$MRPYQ_RECOMMENDATION_DATA_ADDR"
echo "$MRPYQ_RECOMMENDATION_DATA_TIMEOUT_MS"
```

没有 `MRPYQ_RECOMMENDATION_DATA_ADDR` 时，当前实现会使用 Disabled Client，明确返回未配置错误。

### 7.7 Business Feed 返回 `data_loss`
...
优先修上游数据合同，不要在 Home Mixer 里静默吞掉错误。

### 7.8 Thunder 没有 ready
