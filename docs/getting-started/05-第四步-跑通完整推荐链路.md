...
> **前置条件**：完成 [01-准备环境](./01-准备环境.md)（即 `cargo build` 和 `uv sync --dev --group service` 都成功过）。
...
| `HOME_MIXER_MODE=demo` | home-mixer 装配时注入演示客户端，提供关注列表、行为序列、帖子文本，并允许连整数 Thunder | 默认 `degraded` 要求 `MRPYQ_RECOMMENDATION_DATA_ADDR`（指向 rec-bff），缺失直接启动失败；配上后行为序列走 Redis（无投影数据时为空），会得到 NETWORK + FALLBACK 候选的规则排序全网 Feed 骨架；网络范围只由请求的 `in_network_only` 决定 |
...
- **端口被占用**：脚本会直接报出是哪个 gRPC 口。三个服务分别是 50051 / 50052 / 50053；home-mixer 还听 HTTP 9090（`/healthz`、`/readyz`、`/metrics`，`curl localhost:9090/readyz` 返回 `ready` 就能发请求），thunder 还听 HTTP 8080（空路由）。home-mixer 自己的端口被占时会直接以非零码退出并打印 `failed to bind ... port`。
