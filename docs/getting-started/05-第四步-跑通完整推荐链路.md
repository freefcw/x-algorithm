...
sequenceDiagram
    ...
    HM->>P: PredictNextActions（500 条候选逐条预测行为概率）
    P-->>HM: 每条帖子 19 种行为的概率
    HM->>HM: 加权总分 → 作者多样性衰减 → 网外降权 → Top-K
    HM-->>C: 50 条排好序的帖子
```
...
- **网外降权**：网外帖子的总分乘了 0.5（`OON_WEIGHT_FACTOR`），所以随机权重下两类分数会分层。
- **作者多样性**：同一作者的第 2、3 条帖子分数会衰减（×0.55、×0.325），避免刷屏。
...
cd phoenix && uv run scripts/run_grpc_gateway.py
# 想用第三步训练的模型：
...
| `HOME_MIXER_DEMO=1` | home-mixer 装配时注入演示客户端，提供关注列表、行为序列、帖子文本 | 关注列表为空 → 网内召回空；帖子没有文本 → 全部被过滤，返回 0 条 |
| `PHOENIX_*_GRPC_ADDR` | home-mixer 连接真实的 Phoenix 模型服务 | 相应能力退化为桩：网外召回为空 / 所有行为概率视为 0 |

## 排查

- **返回 0 条帖子**：demo-client 会打印排查提示。最常见原因是 thunder 没用演示模式启动，或 `HOME_MIXER_DEMO=1` 没设置。
- **端口被占用**：脚本会直接报出是哪个端口；三个服务分别用 50051/50052/50053，metrics 用 9090。
