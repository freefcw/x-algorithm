...
| `HOME_MIXER_MODE=demo` | home-mixer 装配时注入演示客户端，提供关注列表、行为序列、帖子文本，并允许连整数 Thunder | 默认 `degraded` 要求 `MRPYQ_RECOMMENDATION_DATA_ADDR`，缺失直接启动失败；即便配上 mrpyq，行为序列与 viewer 资格仍是 disabled，只会得到规则排序的关注流 |
