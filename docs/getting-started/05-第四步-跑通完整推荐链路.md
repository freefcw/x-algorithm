...
| `HOME_MIXER_MODE=demo` | home-mixer 装配时注入演示客户端，提供关注列表、行为序列、帖子文本，并允许连整数 Thunder | 默认 `degraded` 要求 `MRPYQ_RECOMMENDATION_DATA_ADDR`，缺失直接启动失败；配上 mrpyq 后，行为序列仍是 disabled，会得到 NETWORK + FALLBACK 候选的规则排序全网 Feed 骨架；网络范围只由请求的 `in_network_only` 决定 |
