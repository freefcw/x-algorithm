...
上游同步以 [upstream-first maintenance policy](./upstream-sync/upstream-first-maintenance.md) 为维护规则，以 [entrypoint migration map](./upstream-sync/entrypoint-migration-map.md) 为执行顺序，以 [e414c17 capability inventory](./upstream-sync/e414c17-to-mp-capability-inventory.md) 为能力状态和验收证据入口。
