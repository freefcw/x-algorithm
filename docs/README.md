...
先决定你要完成什么，再选一组文档。当前 checkout 已移除旧的单机 JAX Demo、随机权重服务、旧 gRPC gateway 及其启动脚本；生产引擎入口以 `phoenix/README.md` 和三篇 Phoenix 生产文档为准。

## 先走一条主线

| 目标 | 入口 | 结果 |
| --- | --- | --- |
| 验证当前代码 | [当前代码验证入口](./getting-started/) | Rust workspace 与 Phoenix xrex 测试 |
...
| 目录 | 只回答什么问题 | 不要拿它做什么 |
| --- | --- | --- |
| `getting-started/` | 当前代码怎样验证 | 不提供 Demo、gateway 或随机权重启动 |
| `bootstrap/` | 从当前代码到真实系统怎样推进 | 不作为日常代码索引 |
...
- 命令只应维护在对应模块 README、三篇 Phoenix 生产文档或部署 README；其他文档只链接，不复制命令。
...
4. 历史记录可以保留，但必须明确标为历史，不能出现在当前操作路径；历史命令不代表当前代码存在。
