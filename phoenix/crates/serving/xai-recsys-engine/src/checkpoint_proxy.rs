...
mod tests {
    ...
    fn fake_trainer(last_octet: u8) -> (SocketAddr, transport::Channel) {
        let addr: SocketAddr = format!("10.0.0.{last_octet}:8080").parse().unwrap();
    ...
    #[tokio::test]
    async fn assigned_slice_keeps_each_channel_paired_with_its_trainer_address() {
        let connected: Vec<_> = (1..=4).map(fake_trainer).collect();

        let mine = assign_trainer_channels(connected, None, 1, 2).expect("assignment");

        let addrs: Vec<_> = mine.iter().map(|(addr, _)| addr.to_string()).collect();
        assert_eq!(addrs, ["10.0.0.3:8080", "10.0.0.4:8080"]);
    }
}
