...
from runners import ACTIONS, load_embedding_table, load_model_params

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s")
logging.getLogger("jax").setLevel(logging.WARNING)
log = logging.getLogger(__name__)

# Model logits use the zero-based order declared by runners.ACTIONS.
ACTION_TO_INDEX = {action_name: index for index, action_name in enumerate(ACTIONS)}
...
def _snowflake_creation_timestamp_seconds(post_ids):
    """Extract creation timestamps from valid Snowflake post IDs."""
...
def infer_corpus_impression_timestamp(corpus_post_ids):
    """Use the corpus end time as a reproducible offline impression timestamp."""
...
def build_candidate_post_age_timestamps(candidate_post_ids, candidate_len, impression_seconds):
    """Build fixed-shape ranker age features, preserving zero-valued padding."""
    ...
    with np.errstate(over="ignore"):
        for i in range(n):
            for j in range(m):
                ...
                out[i, j] = (
                    0 if ids[i] == 0 else int((int(raw) % (num_buckets - 1)) + 1)
                )
    return out
...
def main():
    ...
    impression_timestamp = (
        args.impression_timestamp
        if args.impression_timestamp is not None
        else infer_corpus_impression_timestamp(corpus_post_ids)
    )
    log.info(
        "  %d posts, repr shape %s, offline impression timestamp %d",
        len(corpus_post_ids),
        corpus_repr.shape,
        impression_timestamp,
    )
    ...
    for i in range(0, TOP_K, cand_len):
        ...
        candidate_impr_ts, candidate_post_creation_ts = build_candidate_post_age_timestamps(
            ret_post_ids[i:j], cand_len, impression_timestamp
        )
        rb = RecsysBatch(
            ...
            candidate_impr_ts=jnp.asarray(candidate_impr_ts),
            candidate_post_creation_ts=jnp.asarray(candidate_post_creation_ts),
        )
        re = RecsysEmbeddings(
