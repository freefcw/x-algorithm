...
#
# Derived from jax-triton (https://github.com/jax-ml/jax-triton);
...
def mha_forward_kernel(
    ...
    *residual_refs,
    ...
    block_d: int,
    block_k: int,
    inverted_sliding_window_sizep1: int,
):
    ...
    seg_q = pl.load(
        segment_ref,
        (pl.dslice(start_q * block_q, block_q),),
    )
    ...
    def body(start_k, carry):
        ...
        k = pl.load(k_ref, (slice(None), pl.dslice(start_k * block_k, block_k)))
        seg_k = pl.load(
            segment_ref,
...
@functools.partial(jax.custom_vjp, nondiff_argnums=list(range(5, 20)))
@functools.partial(
    jax.jit,
    static_argnames=[
        "sm_scale",
        ...
        "interpret",
        "debug",
        "inverted_sliding_window_sizep1",
    ],
)
def mha(
    q,
    ...
    kernel = functools.partial(
        ...
        inverted_sliding_window_sizep1=inverted_sliding_window_sizep1,
    )
...
def _preprocess_backward_kernel(out_ref, dout_ref, l_ref, new_dout_ref, delta_ref, *, block_q: int):
    ...
    do = do / denom[:, None]
    do = jnp.where(denom[:, None] == 0, 0.0, do)
...
def mha_backward_kernel_dkv(
    ...
    *,
    ...
    def inner_loop(start_q, carry):
        ...
        mask = jnp.equal(jnp.zeros_like(jnp.expand_dims(seg_q, axis=-1)), seg_k)
        temp = pl.load(temp_ref, (pl.dslice(start_q * block_q, block_q),))
        temp = jnp.expand_dims(temp, axis=-1)
...
def mha_reference(
    ...
    if cap > 0.0:
        if cap_method == "tanh":
            ...
            logits = logits / (1 + jnp.abs(logits) / cap)
        else:
            raise ValueError("cap_method must be in [tanh, soft_sign]")
    causal_mask = jnp.tril(
        jnp.ones((1, 1, k.shape[1], k.shape[1])), k=-inverted_sliding_window_sizep1
    )
    mask = jnp.logical_or(causal_mask, jnp.eye(q.shape[1])[None, None, :, :])

    logits = jnp.where(mask, logits, -1e30)
    weights = jax.nn.softmax(logits).astype(q.dtype)
